...
fn init(owner: Principal) {
    ic_cdk::println!("Initializing token ledger for owner: {:?}", owner);
    LEDGER.with(|ledger| {
...
fn transfer(to: Principal, amount: u64) -> WalletResult<()> {
    ...
    LEDGER.with(|ledger| {
        ...
        let sender_balance = *l.balances.get(&caller).unwrap_or(&0);
        if sender_balance < amount {
            return Err(WalletError::InsufficientBalance);
        }

        // Deduct tokens from the caller
        l.balances.insert(caller, sender_balance - amount);
        ...
        ic_cdk::println!(
            "Transfer successful: {} tokens from {:?} to {:?}",
            amount, caller, to
        );
...
fn receive_tokens(_from: Principal, amount: u64) -> WalletResult<()> {
    ...
    LEDGER.with(|ledger| {
        ...
        let current_balance = *l.balances.get(&caller).unwrap_or(&0);
        l.balances.insert(caller, current_balance + amount);

        ic_cdk::println!("Received {} tokens for {:?}", amount, caller);
...
fn balance() -> u64 {
    LEDGER.with(|ledger| {
        let l = ledger.borrow();
        let caller = get_caller();
        *l.balances.get(&caller).unwrap_or(&0)
    })
}
...
mod tests {
    ...
    fn test_transfer_tokens() {
        ...
        set_caller(owner);
        receive_tokens(owner, 1000).unwrap();
        ...
        LEDGER.with(|ledger| {
            let l = ledger.borrow();
            assert_eq!(*l.balances.get(&owner).unwrap_or(&0), 500);
            assert_eq!(*l.balances.get(&recipient).unwrap_or(&0), 500);
        });
    }
    ...
    fn test_insufficient_balance() {
        ...
        set_caller(owner);
        let result = transfer(recipient, 100);
